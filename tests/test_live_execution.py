...
def test_cash_deposit_updates_ledger_and_portfolio_cash():
    ...
    with engine.connect() as conn:
        assert conn.execute(text("SELECT COUNT(*) FROM live_cash_ledger")).scalar_one() == 1
        assert conn.execute(
            text("SELECT balance_after FROM live_cash_ledger")
        ).scalar_one() == 1250
        assert conn.execute(
            text("SELECT cash_balance FROM live_cash_balances ORDER BY id DESC LIMIT 1")
        ).scalar_one() == 1250
...
def test_cash_withdrawal_dry_run_does_not_write():
    ...
    with engine.connect() as conn:
        ...
        assert conn.execute(text("SELECT COUNT(*) FROM live_cash_balances")).scalar_one() == 1
...
def test_buy_execution_creates_position_trade_cash_and_ledger_rows():
    ...
    with engine.connect() as conn:
        position = conn.execute(
            text("SELECT ticker, shares, buy_price, is_open FROM live_positions")
        ).mappings().one()
        ...
        assert conn.execute(text("SELECT COUNT(*) FROM live_trade_executions")).scalar_one() == 1
        assert conn.execute(text("SELECT COUNT(*) FROM live_cash_ledger")).scalar_one() == 1
        assert conn.execute(
            text("SELECT cash_balance FROM live_cash_balances ORDER BY id DESC LIMIT 1")
        ).scalar_one() == 499
...
def test_sell_execution_reduces_position_books_tax_and_cash():
    ...
    with engine.connect() as conn:
        assert conn.execute(text("SELECT shares FROM live_positions")).scalar_one() == 6
        assert conn.execute(text("SELECT COUNT(*) FROM live_cash_ledger")).scalar_one() == 2
        assert conn.execute(
            text(
                """
...
def _build_engine():
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("INSERT INTO assets (ticker) VALUES ('AAA'), ('BBB')"))
        conn.execute(
            text(
                """
        ...
        conn.execute(
            text(
                """
                INSERT INTO strategy_config_snapshots (as_of_date, tax_rate)
